...
describe.only('basic table builder', async () => {
  ...
  test('basic delete usages', async () => {
    const { tables: db } = new Morbid<typeof Def, Customization>(Def, new pg.Pool({}));
    expect(db.account.deleteAll().compile().text).toBe('delete from "account";');
    expect(db.account.delete({
      id: '123',
      label: ['1', '2'],
    }).compile()).toMatchObject({
      text: 'delete from "account" where "id" = $1 and "label" in ($2, $3);',
      values: ['123', '1', '2'],
    });
    expect(db.account.delete({
      data: null as any,
      ...
      values: ['123'],
      text: 'delete from "account" where "data" is null and "id" = $1 returning "data", "id";',
    });
  });
  test('basic update usages', async () => {
    ...
    expect(db.account
      ...
      text: 'update "account" set "data" = $1 where "id" = $2 returning "label", "id";',
    });
    expect(db.account
      .updateAll()
      .set({
      ...
      text: 'update "account" set "data" = $1 returning "label", "id";',
    });
    expect(db.account
      .updateAll()
      .set({
      ...
      values: [{ kind: 1, email: 'test@test.io' }],
      text: 'update "account" set "data" = $1;',
    });
    expect(db.account
      .update({
        id: '123',
      ...
      values: [{ kind: 1, email: 'test@test.io' }, '123'],
      text: 'update "account" set "data" = $1 where "id" = $2;',
    });
  });
  test('basic insert usages', async () => {
    ...
    expect(db.account
      ...
      values: ['123', { kind: 1 }, ''],
      text: 'insert into "account" ("id", "data", "label") values ($1, $2, $3) returning "label", "id";',
    });
    expect(db.account
      .insert([{} as any, {
        id: '123',
      ...
      values: ['123', { kind: 1 }, ''],
      text: 'insert into "account" ("id", "data", "label")' +
